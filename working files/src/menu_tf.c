#include "header.h"
  ...
  static const uint8_t input_signals_static[MAX_NAMBER_LANGUAGE][1 + NUMBER_TOTAL_SIGNAL_FOR_RANG + (1 - N_IN_GOOSE)  + (1 - N_IN_MMS) + (1 - N_OUT_LAN) + (3  - NUMBER_UP_SIGNAL_FOR_RANG)][MAX_COL_LCD] = 
#else
  static const uint8_t input_signals_static[MAX_NAMBER_LANGUAGE][1 + NUMBER_TOTAL_SIGNAL_FOR_RANG + (3 - NUMBER_UP_SIGNAL_FOR_RANG)][MAX_COL_LCD] = 
#endif
  {
    {"     Пусто      ", NAME_RANG_RU},
    {"     Пусто      ", NAME_RANG_UA},
...
#if (MODYFIKACIA_VERSII_PZ >= 10)
  static const uint8_t output_signals_static[MAX_NAMBER_LANGUAGE][1 + NUMBER_TOTAL_SIGNAL_FOR_RANG_SMALL + (1 - N_IN_GOOSE)  + (1 - N_IN_MMS) + (1 - N_OUT_LAN) + (1 - NUMBER_UP_SIGNAL_FOR_RANG_SMALL)][MAX_COL_LCD] = 
#else
  static const uint8_t output_signals_static[MAX_NAMBER_LANGUAGE][1 + NUMBER_TOTAL_SIGNAL_FOR_RANG_SMALL + (1 - NUMBER_UP_SIGNAL_FOR_RANG_SMALL)][MAX_COL_LCD] = 
#endif
  {
  ...
  for(int index_1 = 0; index_1 < (1 + NUMBER_TOTAL_SIGNAL_FOR_RANG); index_1++)
  {
    ...
    for(size_t index_2 = 0; index_2 < MAX_COL_LCD; index_2++)
    {
...
#endif
      {
        ...
        else input_signals_tmp[index_1][index_2] = input_signals_static[index_language][index_row][index_2];
      }
    }
  }
  ...
  for(size_t index_1 = 0; index_1 < (1 + NUMBER_TOTAL_SIGNAL_FOR_RANG_SMALL); index_1++)
  {
    ...
    for(size_t index_2 = 0; index_2 < MAX_COL_LCD; index_2++)
    {
...
#endif
      {
        ...
        else output_signals_tmp[index_1][index_2] = output_signals_static[index_language][index_row][index_2];
      }
    }
  }
